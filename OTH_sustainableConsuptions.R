...
# reading data
happiness <- read.spss("D:/programming/R/datasets/college/Happiness-Sustainable-Behaviour.sav", to.data.frame=TRUE)
View(happiness)
...
write.csv(data,"D://programming//R//datasets//college//project_no_acc_issue.csv", row.names = FALSE)

data = read.csv("D://programming//R//datasets//college//project_no_acc_issue.csv")
...
write.csv(final_clean_ds,"D://programming//R//datasets//college//project_no_missing_issue.csv", row.names = FALSE)
...
write.csv(nooutlier_df,"D://programming//R//datasets//college//project_no_outliers.csv", row.names = FALSE)

nooutlier_df = read.csv("D://programming//R//datasets//college//project_no_outliers.csv")
str(nooutlier_df)
ncol(nooutlier_df)
...
# Checking the Purchase opinion based on gender
fact_df %>%
        group_by(sex) %>%
...
# Checking the Transport  opinion based on gender
fact_df %>%
        group_by(sex) %>%
...
# such a way that helps the envionment
...
# solution. 
# For instance, in our dataset about 100% of the variation is explained by the first two dimensions.
...
# Conclusion
# 1.Conserve.Water, Conserve.Energy  is close to 7: people are very much in interested in saving water.
...
#example: Quality of representation of columns
head(columns$cos2, 4)
